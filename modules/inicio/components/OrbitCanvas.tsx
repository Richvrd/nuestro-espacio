...
function drawHeartMode(ctx: CanvasRenderingContext2D, cx: number, cy: number, rx: number, ry: number, t: number, timestamp: number) {
  ...
  aura.addColorStop(0, `rgba(201,169,110,${auraAlpha})`);
  aura.addColorStop(0.5, `rgba(184,117,106,${auraAlpha * 0.5})`);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.fillStyle = `rgba(184,117,106,${0.35 - i * 0.04})`;
    ctx.fill();
  }
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const ty = cy + Math.sin(ta) * ry;
    drawHeart(ctx, tx, ty, 5 - i * 0.3);
    ctx.fillStyle = `rgba(201,169,110,${0.35 - i * 0.04})`;
    ctx.fill();
  }
