...
| **Google Fonts** | ✅ En runtime | Cargadas vía `<link>` en `app/layout.tsx` (Playfair Display, Lora, IBM Plex Mono) |
...
| Variable | Valor | Uso |
|---|---|---|
| `--bg` | `#0c0b10` | Fondo general de la app (casi negro con matiz azul-oscuro) |
| `--surface` | `#131219` | Superficies elevadas: sidebar, modales, cajas |
| `--card` | `#1a1924` | Tarjetas, inputs, contenedores de contenido |
| `--card-high` | `#201f2e` | Tarjetas elevadas/hover, menús desplegables |
...
| `--border-hi` | `rgba(201,169,110,0.25)` | Bordes destacados (hover, tarjetas importantes, modales) |

**Texto:**

| Variable | Valor | Uso |
|---|---|---|
| `--text` | `#f0ece0` | Texto principal (crema claro) |
| `--muted` | `#7a7585` | Texto secundario, metadatos, labels |
| `--dimmed` | `#3e3a4a` | Texto terciario / deshabilitado, placeholders |

**Acentos (paleta romántica dorado/rosa/verdeagua):**

| Variable | Valor | Uso |
|---|---|---|
| `--gold` | `#c9a96e` | **Color principal**. Acentos, botones primarios, activos, valores numéricos, fecha/destacados |
| `--gold-dim` | `#7a6035` | Gold atenuado (hovers, decoración sutil) |
| `--rose` | `#b8756a` | Acento secundario (rosa). Errores, "enamorados", badges de no leído, categorías |
| `--rose-dim` | `#6b3d38` | Rosa atenuado |
| `--teal` | `#5a9ba5` | Verdeagua. Módulo cápsulas/cosmos, categoría "viaje" |

**Otros colores en uso (valores directos, sin token):**

| Contexto | Valor |
|---|---|
| Texto sobre fondo gold (botones primarios) | `#0c0b10` / `#080710` |
| Hover de botón primario (gold claro) | `#d4b47a` / `#d9b87e` |
| Categoría timeline "hito" | `#9b8ec4` (lila) |
| Categoría timeline "primer vez" | `#b8756a` (= `--rose`) |
| Sombra de texto/glow en números | `color-mix(in srgb, var(--gold) 30%, transparent)` |
| Gradientes radiales decorativos | `rgba(201,169,110,0.04–0.08)`, `rgba(184,117,106,0.07)`, `rgba(90,155,165,0.04)` |
| Overlays de modales | `rgba(6,6,15,0.85)` o `rgba(8,7,16,0.85)`, con `backdrop-filter: blur(8–12px)` |

### 2.2 Tipografías (font stacks)

| Variable | Stack | Uso |
|---|---|---|
| `--serif` | `'Playfair Display', Georgia, serif` | Títulos, encabezados, números grandes del contador, sujetos de cartas/cápsulas, firmas. Elegante/romántica |
| `--body` | `'Lora', Georgia, serif` | Cuerpo de texto, párrafos, inputs, descripciones |
| `--mono` | `'IBM Plex Mono', monospace` | Labels, subtítulos, metadatos, fechas, botones, tags, todo lo "técnico/de apoyo" |

Fuentes cargadas en `app/layout.tsx`:
```
Playfair Display: ital,wght@0,400;0,500;1,400;1,500
Lora:            ital,wght@0,300;0,400;1,300;1,400
IBM Plex Mono:   wght@200;300;400
```
...
No hay escala formal; el patrón recurrente es: **títulos serif grandes + labels mono minúsculos con letter-spacing amplio + cuerpo Lora**.

| Rol | Fuente | Tamaño | Extra |
|---|---|---|---|
| Título de página | serif | `2.2rem` | `line-height: 1` |
| Título hero inicio | serif | `clamp(2.5rem, 5vw, 4.5rem)` | |
| Números del contador (hero) | serif | `clamp(2.8rem, 6vw, 4.5rem)` | color gold, text-shadow glow |
...
- Nombre de la pareja (`.sidebar-couple`): serif `1rem`, texto con **gradiente dorado→rosa→dorado animado** (`background-clip: text` + `grad-shimmer 4s`).
- `.sidebar-since` / `.sidebar-subtitle`: mono minúsculo, `letter-spacing` amplio, color `--muted`/`--gold`.
- `.nav-item`: `0.85rem`, color `--muted`; hover → `--text` + fondo `rgba(255,255,255,0.03)`; `.active` → `--gold` + fondo `rgba(201,169,110,0.06)` + **barra izquierda de 3px dorada**.
...
|---|---|---|---|---|
| `.btn-primary` | `--gold` | `#0c0b10` | — | `#d4b47a`, `translateY(-1px)`, sombra `0 4px 20px rgba(201,169,110,0.3)` |
| `.btn-ghost` | transparente | `--muted` | `1px solid var(--border)` | `--text`, borde `--border-hi`, fondo `rgba(201,169,110,0.04)` |
...
- `.form-input` / inputs: fondo `--card`, borde `--border`, `radius: 10px`, body `0.85rem`, `:focus` → borde `--gold`. Placeholder → `--dimmed`.
- `.form-error`: texto `--rose` sobre fondo `rgba(184,117,106,0.08)`.
...
- `.inicio-section-tag`: mono `0.5rem` uppercase, letter-spacing 0.4em, `--dimmed`, con línea de 20px `--gold-dim`.
...
- `.inicio-dot-nav`: navegación por puntos, fixed derecha; `.active` → barra dorada 18px.
...
- `.photo-card`: `aspect-ratio: 1`, **ligera rotación** (0.5deg / -0.8deg en `.odd`) estilo polaroid; hover → `translateY(-4px) scale(1.02)` + sombra dorada.
...
- `.upload-zone`: `aspect-ratio: 1`, borde dashed `--border-hi`, hover → `--gold`.
- Stats (`.gallery-stats-row`): valores serif `1.5rem` gold con glow `rgba(201,169,110,0.25)`.
- Filtros (`.filter-chip`): pills radius 20px; activo → `--rose`.

### 6.3 Cápsulas (`capsula`)
- `.capsulas-grid`: `minmax(240px, 1fr)`, gap 1rem.
- `.capsula-card`: base como `.card`; `.sellada` → borde `rgba(201,169,110,0.15)` + gradiente radial sutil; `.abierta` → borde `0.35` + fondo `linear-gradient(135deg, #1a1924, #1e1c2a)`.
- `.capsula-card.just-opened`: animación `just-opened` (pulso de brillo dorado 1.8s).
- Countdowns: `.cdc-val` serif `1.3rem` gold (segundos en `--gold-dim`); grande en modal `.scb-val` `1.8rem`.
- Reader: `.capsule-reader-body` con `border-left: 2px solid var(--gold-dim)`.
- Cosmos (`.cosmos-*`): acento `--teal` (verdeagua). Items list con hover `rgba(90,155,165,0.25)`. Botones `.btn-cosmos`, `.btn-send-space`, `.btn-restore`.

### 6.4 Cartas (`cartas`)
- **Master-detail**: `.cartas-layout` grid `340px 1fr`, altura completa, sin scroll de página.
- Sidebar de lista: búsqueda, tabs (`.cartas-tab.active` → fondo `--card`, texto `--gold`), items con `border-bottom`, `.selected` → `border-left: 2px solid var(--gold)`.
...
- **Filtro por años con planetas** (`.yf-planet`): círculo 52px con gradiente radial `#252038 → #0f0e18`, anillo 3D `rotateX(72deg)`; activo → glow dorado.
...
- **Cards por intensidad**: gradientes de fondo y border dorado progresivos; badges `.badge-high`/`.badge-vhigh`.
- **Categorías** (`.cat-*`): `primer-vez`=rosa, `viaje`=teal, `celebracion`=gold, `cotidiano`=muted, `hito`=lila `#9b8ec4`.
- Vistas `.tl-view-wrapper.compact` (colapsa descripción/acciones con max-height) y `.expanded`.
- Vista por meses: `.month-header` con línea degradada `--gold-dim → transparent`, `.month-name` serif itálica `1.15rem` gold.
- Foto destacada (`.featured-photo`): `aspect-ratio: 3/4`, hover scale 1.015, badge `--gold`, overlay gradiente.
...
- **Reproductor** (en sidebar): `.audio-player` con track info, seek bar de 3px con thumb dorado 10px, botones circulares 28px, control de volumen. En móvil: `.ap-mobile-trigger` (botón flotante 44px) + `.ap-mobile-overlay` con panel bottom-sheet.
...
- Inputs: fondo `--surface`, `caret-color: var(--gold)`, `:focus` → borde `--border-hi` + ring `0 0 0 3px rgba(201,169,110,0.08)` + **línea animada debajo** (`.login-input-line`, scaleX 0→1).
- Botón: gold `0.7rem` uppercase letter-spacing 0.25em; hover `#d9b87e`.
...
- `.nav-item.active::before`: barra dorada arriba de 100% × 2px (en vez de barra lateral).
...
- **Estrellas fugaces** (`.shooting-star`): `z-index: 1`, con cabeza brillante `rgba(255,245,200,0.9)` y glow.
- **Scrollbar**: ancho 5px, thumb `--gold-dim` (hover `--gold`), track transparente.
...
1. **Siempre** usar las variables CSS; no hardcodear colores/tipografías salvo los valores documentados arriba (hovers de gold, text on gold, gradientes decorativos).
...
- **Otros de UI**: `nextjs-toploader` (barra de progreso superior, color gold `var(--gold)`, 2px).
